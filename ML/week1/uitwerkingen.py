...
def drawGraph(data):
    ...
    # Transpose data zodat we een lijst met alle X waarden hebben en een lijst met alle Y waarden
    transposed_data = np.transpose(data)
    plt.scatter(transposed_data[0], transposed_data[1], c='b')
...
def computeCost(X, y, theta):
    ...
    # Bepaal de error voor elk data punt
    # np.dot(X, theta) doet een dot operatie waar X = [1, x(i)] en theta is [0., 0.]
    # Formule is dan v = theta[0]*X[0] + theta[1]*X[1] voor elk datapunt
    # Dit is te lezen als v = 0. * 1 + 0. * x(i)
    # De SSE is dan de som van het resultaat van (v-y)^2 voor alle datapunten
    J = float((sum((np.dot(X, theta) - y) ** 2)) / (2 * m))
...
def gradientDescent(X, y, theta, alpha, num_iters):
    ...
    while i < num_iters:
        i += 1
        theta = theta - alpha * ((sum((np.dot(X, np.transpose(theta)) - y) * X)) / len(X))
